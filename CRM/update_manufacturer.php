...
try {
    ...
    if (!isset($_SESSION['selected_user']) || !array_key_exists($_SESSION['selected_user'], $users)) {
        error_log('Session-Fehler in update_manufacturer.php: selected_user nicht gesetzt oder ungültig. Session: ' . print_r($_SESSION, true));
    ...
    if (!$id || !$machine || !$manufacturer) {
        throw new Exception('Alle Pflichtfelder (ID, Maschine, Hersteller) müssen ausgefüllt sein.');
    }
    ...
    $stmt = $db->prepare("
        ...
        SET machine = :machine, manufacturer = :manufacturer, contact_person = :contact_person, phone = :phone, notes = :notes, links = :links, last_modified_by = :last_modified_by, last_modified_at = NOW()
        WHERE id = :id
    ");
    $stmt->execute([
        ':machine' => $machine,
    ...
    if (!empty($filesToDelete)) {
        foreach ($filesToDelete as $filePath) {
            $stmt = $db->prepare("SELECT COUNT(*) FROM manufacturer_files WHERE manufacturer_id = ? AND file_path = ?");
            $stmt->execute([$id, $filePath]);
            $count = $stmt->fetchColumn();

            if ($count > 0) {
                $stmt = $db->prepare("DELETE FROM manufacturer_files WHERE manufacturer_id = ? AND file_path = ?");
                $stmt->execute([$id, $filePath]);
    ...
    foreach ($existing_image_descriptions as $file_path => $description) {
        ...
        $stmt = $db->prepare("UPDATE manufacturer_files SET description = ? WHERE manufacturer_id = ? AND file_path = ? AND file_type = 'image'");
        $stmt->execute([$description ?: null, $id, $file_path]);
    }

    foreach ($existing_pdf_descriptions as $file_path => $description) {
        $description = trim($description);
        $stmt = $db->prepare("UPDATE manufacturer_files SET description = ? WHERE manufacturer_id = ? AND file_path = ? AND file_type = 'pdf'");
        $stmt->execute([$description ?: null, $id, $file_path]);
    }
